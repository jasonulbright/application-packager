...
App: Eclipse Temurin JDK 8 (x64)
CMName: Eclipse Temurin JDK 8 (x64)

.SYNOPSIS
    Packages Eclipse Temurin JDK 8 (x64) MSI for MECM.

.DESCRIPTION
    Downloads the latest Eclipse Temurin JDK 8 x64 MSI, stages content to a
...
$AppFolder      = "Temurin JDK 8 (x64)"
$Publisher      = "Eclipse Adoptium"
$AppNamePattern = "Eclipse Temurin JDK 8 - {0} (x64)"
$BaseDownloadRoot = Join-Path $DownloadRoot "TemurinJDK8"
...
function Invoke-StageTemurinJDK8X64 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JDK 8 (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
function Invoke-PackageTemurinJDK8X64 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JDK 8 (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JDK 8 (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageTemurinJDK8X64
    }
    elseif ($PackageOnly) {
        Invoke-PackageTemurinJDK8X64
    }
    else {
        Invoke-StageTemurinJDK8X64
        Invoke-PackageTemurinJDK8X64
