...
$Architecture   = "x64"
$VendorFolder   = "Amazon"
$AppFolder      = "Corretto JDK 8 (x64)"
$Publisher      = "Amazon"
$AppNamePattern = "Amazon Corretto JDK 8 - {0} (x64)"
$BaseDownloadRoot = Join-Path $DownloadRoot "CorrettoJDK8"
...
function Invoke-StageCorrettoJDK8X64 {
    Write-Log ""
    Write-Log ("=" * 60)
...
function Invoke-PackageCorrettoJDK8X64 {
    Write-Log ""
    Write-Log ("=" * 60)
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Amazon Corretto JDK 8 (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageCorrettoJDK8X64
    }
    elseif ($PackageOnly) {
        Invoke-PackageCorrettoJDK8X64
    }
    else {
        Invoke-StageCorrettoJDK8X64
        Invoke-PackageCorrettoJDK8X64
