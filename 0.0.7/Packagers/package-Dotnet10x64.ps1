...
.DESCRIPTION
    Retrieves the latest .NET 10 Desktop Runtime version from the official
...
$BaseDownloadRoot = Join-Path $env:USERPROFILE "Downloads\_AutoPackager\DotNet10DesktopRuntime"
...
function Get-LatestDotNet10Version {
    ...
    try {
        ...
        if (-not $channel -or -not $channel.'latest-runtime') {
            throw "Could not find .NET 10.0 release channel or latest runtime."
        }
...
function New-MECMDotNet10DesktopRuntimeApplication {
    param(
...
"@
        ...
        if (-not (Test-Path -LiteralPath $installPs1Path)) {
            $installPs1 = @"
`$proc = Start-Process "`$PSScriptRoot\$InstallerFileName" -ArgumentList "/install /quiet /norestart" -Wait -PassThru -NoNewWindow
exit `$proc.ExitCode
"@
...
"@
        ...
        if (-not (Test-Path -LiteralPath $uninstallPs1Path)) {
            $uninstallPs1 = @"
`$proc = Start-Process "`$PSScriptRoot\$InstallerFileName" -ArgumentList "/uninstall /quiet /norestart" -Wait -PassThru -NoNewWindow
exit `$proc.ExitCode
"@
...
function Get-DotNet10DesktopRuntimeNetworkAppRoot {
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log ".NET 10 Desktop Runtime (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    $networkAppRoot = Get-DotNet10DesktopRuntimeNetworkAppRoot -FileServerPath $FileServerPath

    $version = Get-LatestDotNet10Version
    if (-not $version) {
        throw "Could not resolve .NET 10 runtime version."
    }
    ...
    New-MECMDotNet10DesktopRuntimeApplication `
        -AppName $appName `
