...
.SYNOPSIS
    Automates downloading the latest Tableau Reader installer (x64) and creating an MECM application.

.DESCRIPTION
    Creates an MECM application for Tableau Reader, using registry-based detection
...
$TableauProducts = @(
    @{
        Name = "Tableau Reader {0}"
        Subfolder = "Reader"
        FileNamePattern = "TableauReader-64bit-{0}.exe"
        ProgramsAndFeaturesNamePrefix = "Tableau 20"
        DisplayNameMustContain = "Reader"
        DetectionFile = "tabreader.exe"
        InstallBatContent = '"%~dp0TableauReader-64bit-{0}.exe" /install /quiet /norestart ACCEPTEULA=1 SENDTELEMETRY=0'
        UninstallBatContent = '"%~dp0TableauReader-64bit-{0}.exe" /uninstall /quiet /norestart'
