...
.SYNOPSIS
    Packages Microsoft Edge Enterprise (x64) for MECM.
.DESCRIPTION
    Downloads the latest Microsoft Edge Enterprise x64 MSI, stages content to a versioned network folder,
    and creates an MECM Application with a Script Deployment Type.
    Detection: msedge.exe file version >= packaged version.
.PARAMETER SiteCode
...
$EdgeNetworkShareRoot = Join-Path $FileServerPath "Applications\Microsoft\Chredge"
...
$Product = @{
    Name = "Microsoft Edge Enterprise - {0}"
    FileName = "MicrosoftEdgeEnterpriseX64.msi"
    InstallBatContent = 'msiexec.exe /i "%~dp0MicrosoftEdgeEnterpriseX64.msi" /qn /norestart'
    UninstallBatContent = 'msiexec.exe /x "%~dp0MicrosoftEdgeEnterpriseX64.msi" /qn /norestart'
    DetectionFile = "msedge.exe"
    DetectionPath = "C:\Program Files (x86)\Microsoft\Edge\Application"
    DetectionStagedPath = "C:\Program Files (x86)\Microsoft\EdgeUpdate\Install"
...
try {
    ...
    $downloadFolderName = "EdgeInstallers_${version}"
    ...
    $edgeNetworkPath = Join-Path $EdgeNetworkShareRoot $version
    if (-not (Test-NetworkShareAccess -Path $EdgeNetworkShareRoot)) {
        Write-Error "Network share '${EdgeNetworkShareRoot}' is inaccessible. Skipping Edge application."
        exit 1
    }
    if (-not (Test-Path $edgeNetworkPath)) {
        Write-Host "Creating Edge network directory: ${edgeNetworkPath}"
        New-Item -ItemType Directory -Path $edgeNetworkPath -Force -ErrorAction Stop | Out-Null
    }
    $product = $Product
    $appName = $product.Name -f $version
    $fileName = $product.FileName
    $networkPath = $edgeNetworkPath
    $downloadUrl = $EdgeStableMSIUrl
    ...
    $networkFilePath = Join-Path $networkPath $fileName
        ...
        if (Test-Path $outputPath) {
            $fileInfo = Get-Item -Path $outputPath -ErrorAction Stop
            ...
            try {
                Set-Location C: -ErrorAction Stop
                Write-Host "Set location to C: for download"
        ...
        if (Test-Path $networkFilePath) {
            $fileInfo = Get-Item -Path $networkFilePath -ErrorAction Stop
            ...
            try {
                Set-Location C: -ErrorAction Stop
                Write-Host "Set location to C: for file copy"
                ...
                Write-Error "Failed to copy ${fileName} to '${networkPath}': $($_.Exception.Message)"
                continue
            }
        }
        ...
        $params = @{
            ...
            DetectionPath = $product.DetectionPath
            DetectionFile = $product.DetectionFile
        }
        $params.DetectionStagedPath = $product.DetectionStagedPath
