...
"@
        ...
        if (-not (Test-Path -LiteralPath $installPs1Path)) {
            $installPs1 = @"
`$proc = Start-Process "`$PSScriptRoot\$FileName" -ArgumentList "/VERYSILENT /NORESTART /FORCECLOSEAPPLICATIONS /MERGETASKS=!runcode" -Wait -PassThru -NoNewWindow
exit `$proc.ExitCode
"@
...
"@
        ...
        if (-not (Test-Path -LiteralPath $uninstallPs1Path)) {
            $uninstallPs1 = @"
`$proc = Start-Process "C:\Program Files\Microsoft VS Code\unins000.exe" -ArgumentList "/SILENT" -Wait -PassThru -NoNewWindow
exit `$proc.ExitCode
"@
