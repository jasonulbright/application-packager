<#
Vendor: Amazon
App: Amazon Corretto JDK 11 (x86)
CMName: Amazon Corretto JDK 11 (x86)
...
$Architecture   = "x86"
$VendorFolder   = "Amazon"
$AppFolder      = "Corretto JDK 11 (x86)"
$Publisher      = "Amazon"
$AppNamePattern = "Amazon Corretto JDK 11 - {0} (x86)"
$BaseDownloadRoot = Join-Path $DownloadRoot "CorrettoJDK11x86"
...
function Invoke-StageCorrettoJDK11X86 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Amazon Corretto JDK 11 (x86) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
function Invoke-PackageCorrettoJDK11X86 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Amazon Corretto JDK 11 (x86) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Amazon Corretto JDK 11 (x86) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageCorrettoJDK11X86
    }
    elseif ($PackageOnly) {
        Invoke-PackageCorrettoJDK11X86
    }
    else {
        Invoke-StageCorrettoJDK11X86
        Invoke-PackageCorrettoJDK11X86
