...
App: Eclipse Temurin JRE 11 (x86)
CMName: Eclipse Temurin JRE 11 (x86)
VendorUrl: https://adoptium.net/temurin/releases/

.SYNOPSIS
    Packages Eclipse Temurin JRE 11 (x86) MSI for MECM.

.DESCRIPTION
    Downloads the latest Eclipse Temurin JRE 11 x86 MSI, stages content to a
...
$ImageType      = "jre"
$Architecture   = "x86"
$VendorFolder   = "Eclipse Adoptium"
$AppFolder      = "Temurin JRE 11 (x86)"
$Publisher      = "Eclipse Adoptium"
$AppNamePattern = "Eclipse Temurin JRE 11 - {0} (x86)"
$BaseDownloadRoot = Join-Path $DownloadRoot "TemurinJRE11x86"
...
function Invoke-StageTemurinJRE11X86 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JRE 11 (x86) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    $release = Get-LatestTemurinRelease -FeatureVersion $FeatureVersion -ImageType $ImageType -Architecture $Architecture
...
function Invoke-PackageTemurinJRE11X86 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JRE 11 (x86) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $rel = Get-LatestTemurinRelease -FeatureVersion $FeatureVersion -ImageType $ImageType -Architecture $Architecture -Quiet
        if (-not $rel) { exit 1 }
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JRE 11 (x86) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageTemurinJRE11X86
    }
    elseif ($PackageOnly) {
        Invoke-PackageTemurinJRE11X86
    }
    else {
        Invoke-StageTemurinJRE11X86
        Invoke-PackageTemurinJRE11X86
