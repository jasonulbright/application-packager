...
App: Eclipse Temurin JDK 11 (x64)
CMName: Eclipse Temurin JDK 11 (x64)
VendorUrl: https://adoptium.net/temurin/releases/

.SYNOPSIS
    Packages Eclipse Temurin JDK 11 (x64) MSI for MECM.

.DESCRIPTION
    Downloads the latest Eclipse Temurin JDK 11 x64 MSI, stages content to a
...
$AppFolder      = "Temurin JDK 11 (x64)"
$Publisher      = "Eclipse Adoptium"
$AppNamePattern = "Eclipse Temurin JDK 11 - {0} (x64)"
$BaseDownloadRoot = Join-Path $DownloadRoot "TemurinJDK11"
...
function Invoke-StageTemurinJDK11X64 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JDK 11 (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
function Invoke-PackageTemurinJDK11X64 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JDK 11 (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JDK 11 (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageTemurinJDK11X64
    }
    elseif ($PackageOnly) {
        Invoke-PackageTemurinJDK11X64
    }
    else {
        Invoke-StageTemurinJDK11X64
        Invoke-PackageTemurinJDK11X64
