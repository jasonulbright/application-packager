...
App: Eclipse Temurin JRE 11 (x64)
CMName: Eclipse Temurin JRE 11 (x64)
...
.SYNOPSIS
    Packages Eclipse Temurin JRE 11 (x64) MSI for MECM.

.DESCRIPTION
    Downloads the latest Eclipse Temurin JRE 11 x64 MSI, stages content to a
...
$AppFolder      = "Temurin JRE 11 (x64)"
$Publisher      = "Eclipse Adoptium"
$AppNamePattern = "Eclipse Temurin JRE 11 - {0} (x64)"
$BaseDownloadRoot = Join-Path $DownloadRoot "TemurinJRE11"
...
function Invoke-StageTemurinJRE11X64 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JRE 11 (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
function Invoke-PackageTemurinJRE11X64 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JRE 11 (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JRE 11 (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageTemurinJRE11X64
    }
    elseif ($PackageOnly) {
        Invoke-PackageTemurinJRE11X64
    }
    else {
        Invoke-StageTemurinJRE11X64
        Invoke-PackageTemurinJRE11X64
