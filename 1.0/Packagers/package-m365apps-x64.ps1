...
.DESCRIPTION
    ...
    source files for M365 Apps for Enterprise (x64) from the Semi-Annual Enterprise
    Channel, stages content to a versioned local folder with file-based detection
...
$AppFolder       = "M365 Apps for Enterprise (x64)"
$BaseDownloadRoot = Join-Path $DownloadRoot "M365Apps-x64"
...
function Get-M365VersionFromCdn {
    <#
    .SYNOPSIS
        Queries the Office CDN for the current Semi-Annual Enterprise Channel version.
    #>
    param([switch]$Quiet)
...
function Get-OdtSetupExe {
    <#
    .SYNOPSIS
        Downloads the Office Deployment Tool setup.exe from the Office CDN.
    #>
...
function Invoke-StageM365Apps {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "M365 Apps for Enterprise (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    # --- Copy setup.exe into content folder ---
    ...
    # --- Write install.xml (silent, offline, for endpoint) ---
    ...
    $appName   = "M365 Apps for Enterprise - $version (x64)"
    $publisher = "Microsoft"
...
function Invoke-PackageM365Apps {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "M365 Apps for Enterprise (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "M365 Apps for Enterprise (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
