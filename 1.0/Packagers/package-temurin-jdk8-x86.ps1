...
.DESCRIPTION
    Downloads the latest Eclipse Temurin JDK 8 x86 MSI, stages content to a
...
$AppFolder      = "Temurin JDK 8 (x86)"
$Publisher      = "Eclipse Adoptium"
$AppNamePattern = "Eclipse Temurin JDK 8 - {0} (x86)"
$BaseDownloadRoot = Join-Path $DownloadRoot "TemurinJDK8x86"
...
function Invoke-StageTemurinJDK8X86 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JDK 8 (x86) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
function Invoke-PackageTemurinJDK8X86 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JDK 8 (x86) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JDK 8 (x86) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageTemurinJDK8X86
    }
    elseif ($PackageOnly) {
        Invoke-PackageTemurinJDK8X86
    }
    else {
        Invoke-StageTemurinJDK8X86
        Invoke-PackageTemurinJDK8X86
