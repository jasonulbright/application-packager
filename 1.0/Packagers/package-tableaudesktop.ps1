...
.DESCRIPTION
    Downloads the latest Tableau Desktop x64 installer from the Tableau CDN,
    ...
    Tableau Desktop requires a license (subscription or perpetual). The
    installer is the same for trial and licensed deployments; licensing is
    handled via online sign-in after installation.

    Detection uses file version on tableau.exe in the version-specific install
    directory (e.g., C:\Program Files\Tableau\Tableau 2025.3).
...
function Invoke-StageTableauDesktop {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Tableau Desktop (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    # URL uses hyphens: TableauDesktop-64bit-2025-3-3.exe
    $versionHyphenated = $version -replace '\.', '-'
    $installerFileName = "TableauDesktop-64bit-${versionHyphenated}.exe"
    ...
    if (-not (Test-Path -LiteralPath $localExe)) {
        Write-Log "Downloading Tableau Desktop..."
    ...
    $installContent = @(
        ...
        '$proc = Start-Process -FilePath $exePath -ArgumentList @(''/install'', ''/quiet'', ''/norestart'', ''ACCEPTEULA=1'', ''REMOVEINSTALLEDAPP=1'', ''SENDTELEMETRY=0'') -Wait -PassThru -NoNewWindow',
        'exit $proc.ExitCode'
    ) -join "`r`n"
    ...
    # Tableau Desktop installs to C:\Program Files\Tableau\Tableau <year>.<quarter>
    $versionParts = $version.Split('.')
    $yearQuarter = "{0}.{1}" -f $versionParts[0], $versionParts[1]
    $detectionPath = "{0}\Tableau\Tableau {1}" -f $env:ProgramFiles, $yearQuarter

    $appName   = "Tableau Desktop $version (x64)"
    $publisher = "Salesforce (Tableau)"

    Write-Log "Detection path               : $detectionPath"
    Write-Log "Detection file               : tableau.exe"
    Write-Log ""
...
function Invoke-PackageTableauDesktop {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Tableau Desktop (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Tableau Desktop (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
