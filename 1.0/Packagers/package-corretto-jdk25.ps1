...
.SYNOPSIS
    Packages Amazon Corretto JDK 25 (x64) MSI for MECM.

.DESCRIPTION
    Downloads the latest Amazon Corretto JDK 25 x64 MSI, stages content to a
...
$Architecture   = "x64"
$VendorFolder   = "Amazon"
$AppFolder      = "Corretto JDK 25"
$Publisher      = "Amazon"
$AppNamePattern = "Amazon Corretto JDK 25 - {0} (x64)"
$BaseDownloadRoot = Join-Path $DownloadRoot "CorrettoJDK25"
...
function Invoke-StageCorrettoJDK25 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Amazon Corretto JDK 25 (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
function Invoke-PackageCorrettoJDK25 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Amazon Corretto JDK 25 (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Amazon Corretto JDK 25 (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageCorrettoJDK25
    }
    elseif ($PackageOnly) {
        Invoke-PackageCorrettoJDK25
    }
    else {
        Invoke-StageCorrettoJDK25
        Invoke-PackageCorrettoJDK25
