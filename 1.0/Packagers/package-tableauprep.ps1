...
.DESCRIPTION
    ...
    Detection uses file version on tableau-prep-builder.exe in the version-specific
    install directory (e.g., C:\Program Files\Tableau\Tableau Prep Builder 2025.3).
...
function Invoke-StageTableauPrep {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Tableau Prep Builder (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    # Tableau Prep installs to C:\Program Files\Tableau\Tableau Prep Builder <year>.<quarter>
    $versionParts = $version.Split('.')
    $yearQuarter = "{0}.{1}" -f $versionParts[0], $versionParts[1]
    $detectionPath = "{0}\Tableau\Tableau Prep Builder {1}" -f $env:ProgramFiles, $yearQuarter

    $appName   = "Tableau Prep Builder $version (x64)"
    $publisher = "Salesforce (Tableau)"

    Write-Log "Detection path               : $detectionPath"
    Write-Log "Detection file               : tableau-prep-builder.exe"
    Write-Log ""
    ...
    Write-StageManifest -Path $manifestPath -ManifestData @{
        ...
        Detection       = @{
            ...
            FileName      = "tableau-prep-builder.exe"
...
function Invoke-PackageTableauPrep {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Tableau Prep Builder (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
