<#
Vendor: Amazon
App: Amazon Corretto JDK 11 (x64)
CMName: Amazon Corretto JDK 11 (x64)
VendorUrl: https://aws.amazon.com/corretto/
CPE: cpe:2.3:a:amazon:corretto:*:*:*:*:*:*:*:*
ReleaseNotesUrl: https://github.com/corretto/corretto-11/releases
DownloadPageUrl: https://aws.amazon.com/corretto/

.SYNOPSIS
    Packages Amazon Corretto JDK 11 (x64) MSI for MECM.

.DESCRIPTION
    Downloads the latest Amazon Corretto JDK 11 x64 MSI, stages content to a
...
$Architecture   = "x64"
$VendorFolder   = "Amazon"
$AppFolder      = "Corretto JDK 11 (x64)"
$Publisher      = "Amazon"
$AppNamePattern = "Amazon Corretto JDK 11 - {0} (x64)"
$BaseDownloadRoot = Join-Path $DownloadRoot "CorrettoJDK11"
...
function Invoke-StageCorrettoJDK11X64 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Amazon Corretto JDK 11 (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
function Invoke-PackageCorrettoJDK11X64 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Amazon Corretto JDK 11 (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Amazon Corretto JDK 11 (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageCorrettoJDK11X64
    }
    elseif ($PackageOnly) {
        Invoke-PackageCorrettoJDK11X64
    }
    else {
        Invoke-StageCorrettoJDK11X64
        Invoke-PackageCorrettoJDK11X64
