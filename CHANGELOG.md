...
- GUI version string validation — widened regex from `^\d+(\.\d+){1,3}$` to `^\d+(\.\d+){1,3}(\+\d+)?$` to accept `+build` suffixes (e.g., RStudio `2026.01.1+403`); `Compare-SemVer` strips `+build` before `[version]` parsing
