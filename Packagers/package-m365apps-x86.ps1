<#
Vendor: Microsoft
App: M365 Apps for Enterprise (x86)
...
.SYNOPSIS
    Packages M365 Apps for Enterprise (x86) for MECM using the Office Deployment Tool.

.DESCRIPTION
    Downloads the latest Office Deployment Tool, uses it to download the offline
    source files for M365 Apps for Enterprise (x86) from the configured update
    channel (Monthly Enterprise or Current), stages content to a versioned local
    folder with file-based detection metadata, and creates an MECM Application.
...
$CdnCabUrl       = "$CdnBaseUrl/v32.cab"
$OdtSetupUrl     = "https://officecdn.microsoft.com/pr/wsus/setup.exe"

$VendorFolder    = "Microsoft"
$AppFolder       = "M365 Apps for Enterprise (x86) [$($ch.Tag)]"
$BaseDownloadRoot = Join-Path $DownloadRoot "M365Apps-x86-$($ch.Tag)"
...
function Get-M365VersionFromCdn {
    ...
    try {
        ...
        $version = $vdXml.Version.Available.I320Version
...
function Invoke-StageM365Apps {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "M365 Apps for Enterprise (x86) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    $appName   = "M365 Apps for Enterprise - $version (x86) [$($ch.Display)]"
    $publisher = "Microsoft"
    ...
    Write-StageManifest -Path $manifestPath -ManifestData @{
        AppName         = $appName
        DisplayName     = "M365 Apps for Enterprise (x86)"
        Publisher       = $publisher
...
function Invoke-PackageM365Apps {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "M365 Apps for Enterprise (x86) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "M365 Apps for Enterprise (x86) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageM365Apps
    }
    elseif ($PackageOnly) {
        Invoke-PackageM365Apps
    }
    else {
        Invoke-StageM365Apps
        Invoke-PackageM365Apps
