...
.SYNOPSIS
    Packages Amazon Corretto JDK 21 (x64) MSI for MECM.

.DESCRIPTION
    Downloads the latest Amazon Corretto JDK 21 x64 MSI, stages content to a
...
$Architecture   = "x64"
$VendorFolder   = "Amazon"
$AppFolder      = "Corretto JDK 21"
$Publisher      = "Amazon"
$AppNamePattern = "Amazon Corretto JDK 21 - {0} (x64)"
$BaseDownloadRoot = Join-Path $DownloadRoot "CorrettoJDK21"
...
function Invoke-StageCorrettoJDK21 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Amazon Corretto JDK 21 (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
function Invoke-PackageCorrettoJDK21 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Amazon Corretto JDK 21 (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Amazon Corretto JDK 21 (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageCorrettoJDK21
    }
    elseif ($PackageOnly) {
        Invoke-PackageCorrettoJDK21
    }
    else {
        Invoke-StageCorrettoJDK21
        Invoke-PackageCorrettoJDK21
