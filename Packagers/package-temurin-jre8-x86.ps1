...
App: Eclipse Temurin JRE 8 (x86)
CMName: Eclipse Temurin JRE 8 (x86)
...
.SYNOPSIS
    Packages Eclipse Temurin JRE 8 (x86) MSI for MECM.

.DESCRIPTION
    Downloads the latest Eclipse Temurin JRE 8 x86 MSI, stages content to a
...
$AppFolder      = "Temurin JRE 8 (x86)"
$Publisher      = "Eclipse Adoptium"
$AppNamePattern = "Eclipse Temurin JRE 8 - {0} (x86)"
$BaseDownloadRoot = Join-Path $DownloadRoot "TemurinJRE8x86"
...
function Invoke-StageTemurinJRE8X86 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JRE 8 (x86) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
function Invoke-PackageTemurinJRE8X86 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JRE 8 (x86) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JRE 8 (x86) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageTemurinJRE8X86
    }
    elseif ($PackageOnly) {
        Invoke-PackageTemurinJRE8X86
    }
    else {
        Invoke-StageTemurinJRE8X86
        Invoke-PackageTemurinJRE8X86
