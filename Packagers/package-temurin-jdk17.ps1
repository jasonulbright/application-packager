...
App: Eclipse Temurin JDK 17 (x64)
CMName: Eclipse Temurin JDK 17
...
.SYNOPSIS
    Packages Eclipse Temurin JDK 17 (x64) MSI for MECM.

.DESCRIPTION
    Downloads the latest Eclipse Temurin JDK 17 x64 MSI, stages content to a
...
$AppFolder      = "Temurin JDK 17"
$Publisher      = "Eclipse Adoptium"
$AppNamePattern = "Eclipse Temurin JDK 17 - {0} (x64)"
$BaseDownloadRoot = Join-Path $DownloadRoot "TemurinJDK17"
...
function Invoke-StageTemurinJDK17 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JDK 17 (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
function Invoke-PackageTemurinJDK17 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JDK 17 (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JDK 17 (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageTemurinJDK17
    }
    elseif ($PackageOnly) {
        Invoke-PackageTemurinJDK17
    }
    else {
        Invoke-StageTemurinJDK17
        Invoke-PackageTemurinJDK17
