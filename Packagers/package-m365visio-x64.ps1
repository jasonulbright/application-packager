...
.SYNOPSIS
    Packages M365 Visio (x64) for MECM using the Office Deployment Tool.

.DESCRIPTION
    Downloads the latest Office Deployment Tool, uses it to download the offline
    source files for M365 Visio (x64) from the configured update channel
    ...
    Detection uses VISIO.EXE file version >= packaged version in the
...
$AppFolder       = "M365 Visio (x64) [$($ch.Tag)]"
$BaseDownloadRoot = Join-Path $DownloadRoot "M365Visio-x64-$($ch.Tag)"
...
function Invoke-StageM365Visio {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "M365 Visio (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    $appName   = "M365 Visio - $version (x64) [$($ch.Display)]"
    $publisher = "Microsoft"
...
function Invoke-PackageM365Visio {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "M365 Visio (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "M365 Visio (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageM365Visio
    }
    elseif ($PackageOnly) {
        Invoke-PackageM365Visio
    }
    else {
        Invoke-StageM365Visio
        Invoke-PackageM365Visio
