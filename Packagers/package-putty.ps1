...
.DESCRIPTION
    Downloads the latest PuTTY x64 MSI from the official PuTTY download server,
    ...
    Version detection scrapes the PuTTY download directory listing at
    the.earth.li to identify the latest MSI filename. ARP detection is derived
    from MSI properties (ProductCode becomes registry key, ProductVersion
    becomes DisplayVersion). No temp install needed.
...
.PARAMETER FileServerPath
    ...
    Content is staged under: <FileServerPath>\Applications\PuTTY\PuTTY\<Version>

.PARAMETER DownloadRoot
    Local root folder for staging downloaded installers.
    Each packager creates a subfolder under this path (e.g., <DownloadRoot>\PuTTY).
    Default: C:\temp\ap
...
.PARAMETER GetLatestVersionOnly
    Outputs only the latest available PuTTY version string and exits.
...
# --- Configuration ---
$DownloadDirUrl = "https://the.earth.li/~sgtatham/putty/latest/w64/"

$VendorFolder = "PuTTY"
$AppFolder    = "PuTTY"

$BaseDownloadRoot = Join-Path $DownloadRoot "PuTTY"

# --- Functions ---


function Resolve-PuTTYLatestVersion {
    param([switch]$Quiet)

    Write-Log "PuTTY download directory      : $DownloadDirUrl" -Quiet:$Quiet

    try {
        $html = (curl.exe -L --fail --silent --show-error $DownloadDirUrl) -join "`n"
        if ($LASTEXITCODE -ne 0) { throw "Failed to fetch PuTTY download directory: $DownloadDirUrl" }

        if ($html -match 'putty-64bit-([0-9]+\.[0-9]+)-installer\.msi') {
            $version = $matches[1]
            Write-Log "Latest PuTTY version         : $version" -Quiet:$Quiet
            return $version
        }

        throw "Could not parse PuTTY version from download directory listing."
    }
    catch {
        Write-Log "Failed to resolve PuTTY version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}


function Get-PuTTYDisplayVersion {
...
function Invoke-StagePuTTY {
    ...
    # --- Get version ---
    $version = Resolve-PuTTYLatestVersion
    if (-not $version) { throw "Could not resolve PuTTY version." }

    $msiFileName = "putty-64bit-$version-installer.msi"
    $msiUrl      = "${DownloadDirUrl}${msiFileName}"
    ...
    $productVersionRaw = $props["ProductVersion"]   # e.g. 0.83.0.0
    ...
    $displayVersion = Get-PuTTYDisplayVersion -RawVersion $productVersionRaw
    ...
    if ([string]::IsNullOrWhiteSpace($publisher)) { $publisher = "Simon Tatham" }

    $appName = ("PuTTY - {0} (x64)" -f $displayVersion)
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $v = Resolve-PuTTYLatestVersion -Quiet
        if (-not $v) { exit 1 }
        Write-Output $v
        exit 0
    }
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "PuTTY (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    Write-Log "DownloadDirUrl               : $DownloadDirUrl"
    Write-Log ""
