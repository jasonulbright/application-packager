...
App: Eclipse Temurin JDK 21 (x64)
CMName: Eclipse Temurin JDK 21
...
.SYNOPSIS
    Packages Eclipse Temurin JDK 21 (x64) MSI for MECM.

.DESCRIPTION
    Downloads the latest Eclipse Temurin JDK 21 x64 MSI, stages content to a
...
$AppFolder      = "Temurin JDK 21"
$Publisher      = "Eclipse Adoptium"
$AppNamePattern = "Eclipse Temurin JDK 21 - {0} (x64)"
$BaseDownloadRoot = Join-Path $DownloadRoot "TemurinJDK21"
...
function Invoke-StageTemurinJDK21 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JDK 21 (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
function Invoke-PackageTemurinJDK21 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JDK 21 (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JDK 21 (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageTemurinJDK21
    }
    elseif ($PackageOnly) {
        Invoke-PackageTemurinJDK21
    }
    else {
        Invoke-StageTemurinJDK21
        Invoke-PackageTemurinJDK21
