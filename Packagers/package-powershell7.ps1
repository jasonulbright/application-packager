...
.PARAMETER FileServerPath
    ...
    Content is staged under: <FileServerPath>\Applications\Microsoft\PowerShell 7 (x64)\<Version>

.PARAMETER DownloadRoot
    Local root folder for staging downloaded installers.
    Each packager creates a subfolder under this path (e.g., <DownloadRoot>\PowerShell7).
    Default: C:\temp\ap
...
$AppFolder    = "PowerShell 7 (x64)"

$BaseDownloadRoot = Join-Path $DownloadRoot "PowerShell7"
...
function Get-LatestPowerShell7Version {
    ...
    try {
        ...
        Write-Log "Latest PowerShell 7 version  : $version" -Quiet:$Quiet
        return $version
    }
    catch {
        Write-Log "Failed to get PowerShell 7 version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StagePowerShell7 {
    Write-Log ""
    Write-Log ("=" * 60)
    ...
    # --- Get version ---
    $version = Get-LatestPowerShell7Version
    if (-not $version) { throw "Could not resolve PowerShell 7 version." }
    ...
    $appName = "PowerShell 7 - $productVersionRaw (x64)"
...
function Invoke-PackagePowerShell7 {
    Write-Log ""
    Write-Log ("=" * 60)
    ...
    if (-not $msiFiles -or $msiFiles.Count -eq 0) {
        throw "No staged PowerShell 7 MSI found - run Stage phase first."
    }
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $v = Get-LatestPowerShell7Version -Quiet
        if (-not $v) { exit 1 }
        Write-Output $v
        exit 0
    }
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "PowerShell 7 (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StagePowerShell7
    }
    elseif ($PackageOnly) {
        Invoke-PackagePowerShell7
    }
    else {
        Invoke-StagePowerShell7
        Invoke-PackagePowerShell7
