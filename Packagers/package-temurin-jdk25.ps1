...
App: Eclipse Temurin JDK 25 (x64)
CMName: Eclipse Temurin JDK 25
...
.SYNOPSIS
    Packages Eclipse Temurin JDK 25 (x64) MSI for MECM.

.DESCRIPTION
    Downloads the latest Eclipse Temurin JDK 25 x64 MSI, stages content to a
...
$AppFolder      = "Temurin JDK 25"
$Publisher      = "Eclipse Adoptium"
$AppNamePattern = "Eclipse Temurin JDK 25 - {0} (x64)"
$BaseDownloadRoot = Join-Path $DownloadRoot "TemurinJDK25"
...
function Invoke-StageTemurinJDK25 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JDK 25 (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
function Invoke-PackageTemurinJDK25 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JDK 25 (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JDK 25 (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageTemurinJDK25
    }
    elseif ($PackageOnly) {
        Invoke-PackageTemurinJDK25
    }
    else {
        Invoke-StageTemurinJDK25
        Invoke-PackageTemurinJDK25
