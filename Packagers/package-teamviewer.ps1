...
.DESCRIPTION
    ...
    GetLatestVersionOnly downloads the ZIP, extracts version from the MSI, and
    cleans up. No lighter-weight version API is available.
...
.PARAMETER FileServerPath
    ...
    Content is staged under: <FileServerPath>\Applications\TeamViewer\TeamViewer\<Version>

.PARAMETER DownloadRoot
    Local root folder for staging downloaded installers.
    Each packager creates a subfolder under this path (e.g., <DownloadRoot>\TeamViewer).
    Default: C:\temp\ap
...
# --- Configuration ---
$ZipDownloadUrl = "https://download.teamviewer.com/download/version_15x/TeamViewer_MSI64.zip"
$ZipFileName    = "TeamViewer_MSI64.zip"

$VendorFolder = "TeamViewer"
$AppFolder    = "TeamViewer"

$BaseDownloadRoot = Join-Path $DownloadRoot "TeamViewer"

# --- Functions ---


function Get-TeamViewerMsi {
    <#
    .SYNOPSIS
        Downloads and extracts the TeamViewer MSI from the ZIP archive.
        Returns the path to the extracted MSI.
    #>
    param([switch]$Quiet)
    ...
    if (-not $msiFile) { throw "No MSI found inside TeamViewer ZIP." }

    Write-Log "Extracted MSI                : $($msiFile.FullName)" -Quiet:$Quiet
...
function Get-LatestTeamViewerVersion {
    ...
    Write-Log "TeamViewer ZIP URL           : $ZipDownloadUrl" -Quiet:$Quiet

    try {
        $msiPath = Get-TeamViewerMsi -Quiet:$Quiet
        ...
        Write-Log "Latest TeamViewer version    : $version" -Quiet:$Quiet
        return $version
    }
    catch {
        Write-Log "Failed to get TeamViewer version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageTeamViewer {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "TeamViewer (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    $props = Get-MsiPropertyMap -MsiPath $extractedMsiPath
    ...
    if (-not (Test-Path -LiteralPath $stagedMsi)) {
        Copy-Item -LiteralPath $extractedMsiPath -Destination $stagedMsi -Force -ErrorAction Stop
    ...
    $appName = "TeamViewer $productVersionRaw"
...
function Invoke-PackageTeamViewer {
    Write-Log ""
    Write-Log ("=" * 60)
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $v = Get-LatestTeamViewerVersion -Quiet
        if (-not $v) { exit 1 }
        Write-Output $v
        exit 0
    }
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "TeamViewer (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageTeamViewer
    }
    elseif ($PackageOnly) {
        Invoke-PackageTeamViewer
    }
    else {
        Invoke-StageTeamViewer
        Invoke-PackageTeamViewer
