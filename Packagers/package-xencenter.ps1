...
CPE: cpe:2.3:a:cloud_software_group:xencenter:*:*:*:*:*:*:*:*
ReleaseNotesUrl: https://docs.xenserver.com/en-us/xenserver/8/whats-new
DownloadPageUrl: https://www.xenserver.com/downloads

.SYNOPSIS
    Packages XenCenter MSI for MECM.

.DESCRIPTION
    Scrapes the XenServer downloads page for the latest XenCenter MSI URL,
    downloads it, stages content to a versioned local folder with ARP
...
$MsiFileName      = "XenCenter.msi"

# --- Functions ---


function Resolve-XenCenterMsiUrl {
    param([switch]$Quiet)
    ...
    try {
        ...
        # Match: https://downloads.xenserver.com/xencenter/<version>/XenCenter-<version>.msi
        $rx = [regex]'href\s*=\s*"(?<url>https://downloads\.xenserver\.com/xencenter/(?<ver>[^/]+)/XenCenter[^"]*\.msi)"'
        $m = $rx.Match($html)

        if (-not $m.Success) {
            throw "Could not locate XenCenter MSI link on the downloads page."
        }
        ...
        Write-Log "Failed to resolve XenCenter MSI URL: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageXenCenter {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "XenCenter - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    # --- Download ---
    $msiUrl = Resolve-XenCenterMsiUrl
    if (-not $msiUrl) { throw "Could not resolve XenCenter MSI download URL." }
    ...
    # XenCenter uses /q (basic quiet) not /qn
    $installPs1 = @(
    ...
    $appName = "XenCenter - $productVersionRaw"
...
function Invoke-PackageXenCenter {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "XenCenter - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $msiUrl = Resolve-XenCenterMsiUrl -Quiet
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "XenCenter Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
