...
.SYNOPSIS
    Packages M365 Project (x86) for MECM using the Office Deployment Tool.
...
$AppFolder       = "M365 Project (x86) [$($ch.Tag)]"
$BaseDownloadRoot = Join-Path $DownloadRoot "M365Project-x86-$($ch.Tag)"
...
function Invoke-StageM365Project {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "M365 Project (x86) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
function Invoke-PackageM365Project {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "M365 Project (x86) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "M365 Project (x86) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
