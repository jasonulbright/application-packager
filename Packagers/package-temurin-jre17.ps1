...
App: Eclipse Temurin JRE 17 (x64)
CMName: Eclipse Temurin JRE 17
...
.SYNOPSIS
    Packages Eclipse Temurin JRE 17 (x64) MSI for MECM.

.DESCRIPTION
    Downloads the latest Eclipse Temurin JRE 17 x64 MSI, stages content to a
...
$AppFolder      = "Temurin JRE 17"
$Publisher      = "Eclipse Adoptium"
$AppNamePattern = "Eclipse Temurin JRE 17 - {0} (x64)"
$BaseDownloadRoot = Join-Path $DownloadRoot "TemurinJRE17"
...
function Invoke-StageTemurinJRE17 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JRE 17 (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
function Invoke-PackageTemurinJRE17 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JRE 17 (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JRE 17 (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageTemurinJRE17
    }
    elseif ($PackageOnly) {
        Invoke-PackageTemurinJRE17
    }
    else {
        Invoke-StageTemurinJRE17
        Invoke-PackageTemurinJRE17
