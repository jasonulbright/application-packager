<#
Vendor: Amazon
App: Amazon Corretto JDK 8 (x86)
CMName: Amazon Corretto JDK 8 (x86)
...
.SYNOPSIS
    Packages Amazon Corretto JDK 8 (x86) MSI for MECM.

.DESCRIPTION
    Downloads the latest Amazon Corretto JDK 8 x86 MSI, stages content to a
...
$Architecture   = "x86"
$VendorFolder   = "Amazon"
$AppFolder      = "Corretto JDK 8 (x86)"
$Publisher      = "Amazon"
$AppNamePattern = "Amazon Corretto JDK 8 - {0} (x86)"
$BaseDownloadRoot = Join-Path $DownloadRoot "CorrettoJDK8x86"
...
function Invoke-StageCorrettoJDK8X86 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Amazon Corretto JDK 8 (x86) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
function Invoke-PackageCorrettoJDK8X86 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Amazon Corretto JDK 8 (x86) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Amazon Corretto JDK 8 (x86) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageCorrettoJDK8X86
    }
    elseif ($PackageOnly) {
        Invoke-PackageCorrettoJDK8X86
    }
    else {
        Invoke-StageCorrettoJDK8X86
        Invoke-PackageCorrettoJDK8X86
