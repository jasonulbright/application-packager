...
$AppFolder       = "M365 Apps for Enterprise (x64) [$($ch.Tag)]"
$BaseDownloadRoot = Join-Path $DownloadRoot "M365Apps-x64-$($ch.Tag)"
...
function Get-M365VersionFromCdn {
    <#
    .SYNOPSIS
        Queries the Office CDN for the current channel version.
    #>
    param([switch]$Quiet)
...
function Invoke-StageM365Apps {
    ...
    $appName   = "M365 Apps for Enterprise - $version (x64) [$($ch.Display)]"
    $publisher = "Microsoft"
    ...
    Write-StageManifest -Path $manifestPath -ManifestData @{
        AppName         = $appName
        DisplayName     = "M365 Apps for Enterprise (x64)"
        Publisher       = $publisher
