...
.DESCRIPTION
    Downloads the latest AnyDesk EXE from the static download URL, stages content
    ...
    NOTE: AnyDesk's MSI download is no longer publicly available. This packager
    uses the EXE installer with --install --silent flags. The version is read
    from the downloaded EXE's file version info.
...
.PARAMETER GetLatestVersionOnly
    Downloads the AnyDesk EXE, reads the file version, outputs the version
...
# --- Configuration ---
$ExeDownloadUrl    = "https://download.anydesk.com/AnyDesk.exe"
...
function Get-LatestAnyDeskVersion {
    ...
    Write-Log "AnyDesk download URL         : $ExeDownloadUrl" -Quiet:$Quiet
    ...
    try {
        ...
        if (-not (Test-Path -LiteralPath $localExe)) {
            Write-Log "Downloading AnyDesk EXE to read version..." -Quiet:$Quiet
        ...
        if ([string]::IsNullOrWhiteSpace($version)) { throw "Could not read file version from AnyDesk.exe." }

        Write-Log "Latest AnyDesk version       : $version" -Quiet:$Quiet
        return $version
    }
    catch {
        Write-Log "Failed to get AnyDesk version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageAnyDesk {
    ...
    # AnyDesk installs to Program Files (x86) by default
    $wrapperContent = New-ExeWrapperContent `
        -InstallerFileName $InstallerFileName `
        -InstallArgs "'--install', '--silent'" `
        -UninstallCommand 'C:\Program Files (x86)\AnyDesk\AnyDesk.exe' `
    ...
    $detectionPath = "{0}\AnyDesk" -f ${env:ProgramFiles(x86)}

    $appName   = "AnyDesk $version"
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $v = Get-LatestAnyDeskVersion -Quiet
        if (-not $v) { exit 1 }
        Write-Output $v
        exit 0
    }
