...
App: Eclipse Temurin JRE 25 (x64)
CMName: Eclipse Temurin JRE 25
...
.SYNOPSIS
    Packages Eclipse Temurin JRE 25 (x64) MSI for MECM.

.DESCRIPTION
    Downloads the latest Eclipse Temurin JRE 25 x64 MSI, stages content to a
...
$AppFolder      = "Temurin JRE 25"
$Publisher      = "Eclipse Adoptium"
$AppNamePattern = "Eclipse Temurin JRE 25 - {0} (x64)"
$BaseDownloadRoot = Join-Path $DownloadRoot "TemurinJRE25"
...
function Invoke-StageTemurinJRE25 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JRE 25 (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
function Invoke-PackageTemurinJRE25 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JRE 25 (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Eclipse Temurin JRE 25 (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageTemurinJRE25
    }
    elseif ($PackageOnly) {
        Invoke-PackageTemurinJRE25
    }
    else {
        Invoke-StageTemurinJRE25
        Invoke-PackageTemurinJRE25
